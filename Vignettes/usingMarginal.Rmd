...
transitions=matrix(0,nrow = 10,ncol=10)
...
df=dfh
ages=c(40:81)
s=statusarray(df_frame = data.table(dfh),ages = ages,nstates = nstates)
```

```{r}
s=readRDS(file = "~/multistate2/output/statusarray.rds")
dim(s)
head(s[,18,])
```
...
```{r,eval=F}
ages=40:80
covariates="f.31.0.0+cad.prs"
mfit=modelfitfun(ages = ages,transit_mat=transitions,covariates,df = dfh,statusarray = s)
```
...
ages=40:80
mfit=readRDS("~/multistate2/output/mfit.rds")
