...
modelfitfun=function(ages,transit_mat,covariates,df,statusarray){
    ...
    for (end_state in states) {
      ...
      for (start_state in states[-c(5,6)]) {
        ...
        if(transit_mat[start_state,end_state] == 1){
          ...
          end_state_list[[start_state]] <- summary(modfit)$coefficients # The variable from your actual model summary
        }
      }
